...
export default function Landing() {
    ...
    return (
      <div style={{ marginTop: "10px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
          <span style={{ fontSize: "0.78rem", fontWeight: "700", color: level.color, background: level.bg, padding: "2px 10px", borderRadius: "20px" }}>
            {level.label}
          </span>
          {strength === 4 && <span style={{ fontSize: "0.78rem", color: "#38A169" }}>✅ Great password!</span>}
        </div>
        {tips.length > 0 && (
          <div style={{ background: "rgba(0,0,0,0.04)", borderRadius: "8px", padding: "8px 12px", fontSize: "0.78rem", color: theme.subtext }}>
            💡 Add: {tips.join(", ")}
          </div>
        )}
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.background, fontFamily: "'Segoe UI', sans-serif", transition: "all 1.5s ease" }}>
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div style={{ width: "40px", height: "40px", borderRadius: "12px", background: theme.buttonBg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", boxShadow: theme.shadow }}>💰</div>
          <span style={{ fontSize: "1.3rem", fontWeight: "800", color: theme.text }}>ExpenseTracker</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span style={{ fontSize: "1.2rem" }}>{theme.icon}</span>
          <span style={{ color: theme.subtext, fontSize: "0.9rem", fontWeight: "500" }}>{theme.greeting}</span>
        </div>
      </nav>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: "100vh", paddingTop: "72px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 48px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.3)", borderRadius: "20px", padding: "6px 16px", marginBottom: "24px", width: "fit-content", border: "1px solid rgba(255,255,255,0.4)" }}>
          ...
          <h1 style={{ fontSize: "clamp(2.5rem, 4vw, 3.5rem)", fontWeight: "900", color: theme.text, lineHeight: 1.15, marginBottom: "20px", letterSpacing: "-0.02em" }}>
            Take Control of<br />
            <span style={{ background: `linear-gradient(135deg, ${theme.primary}, ${theme.secondary})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", display: "inline-block", color: "transparent" }}>
              Your Finances
            </span>
          </h1>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", maxWidth: "480px" }}>
            {features.map((f, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.4)", backdropFilter: "blur(10px)", borderRadius: "16px", padding: "16px", border: "1px solid rgba(255,255,255,0.5)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 48px" }}>
          <div style={{ width: "100%", maxWidth: "440px", background: "rgba(255,255,255,0.7)", backdropFilter: "blur(30px)", borderRadius: "28px", padding: "44px", boxShadow: "0 32px 64px rgba(0,0,0,0.12)", border: "1px solid rgba(255,255,255,0.8)" }}>
            ...
            {/* Messages */}
            {error && (
              <div style={{ background: "#FFF5F5", border: "1px solid #FEB2B2", color: "#C53030", padding: "12px 16px", borderRadius: "12px", marginBottom: "20px", fontSize: "0.85rem", fontWeight: "500" }}>
                ⚠️ {error}
              </div>
            )}
            {success && (
              <div style={{ background: "#F0FFF4", border: "1px solid #9AE6B4", color: "#276749", padding: "12px 16px", borderRadius: "12px", marginBottom: "20px", fontSize: "0.85rem", fontWeight: "500" }}>
                {success}
              </div>
            )}
            ...
            {isLogin && (
              <form onSubmit={handleLogin}>
                ...
                <button type="submit" disabled={loading} style={{ width: "100%", padding: "15px", borderRadius: "14px", border: "none", background: `linear-gradient(135deg, ${theme.primary}, ${theme.secondary})`, color: "#fff", fontSize: "1rem", fontWeight: "700", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1, boxShadow: `0 8px 24px ${theme.primary}40` }}>
            ...
            {!isLogin && (
              <form onSubmit={handleRegister}>
                ...
                <button type="submit" disabled={loading} style={{ width: "100%", padding: "15px", borderRadius: "14px", border: "none", marginTop: "20px", background: `linear-gradient(135deg, ${theme.primary}, ${theme.secondary})`, color: "#fff", fontSize: "1rem", fontWeight: "700", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1, boxShadow: `0 8px 24px ${theme.primary}40` }}>
